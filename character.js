...
{
    constructor(jsonIn, _guild,_index, _parObj, _clientID, _gmID, _mainChannel, _utility, msg){
        ...
        this.clientID=_clientID; this.gmID=_gmID;           this.mainChannel=_mainChannel;  this.utility=_utility;
        this.running=false;     this.error=false;           this.initialised=false;         this.items=[];          
        this.cash=0;            this.examineItems=[];      
        ...
        for(var jObj in jsonIn){
            switch(jObj.toLowerCase()){
                ...
                case 'nickname': case 'description': this.charName=jsonIn[jObj]; break;
                case 'cash': this.cash = Number(jsonIn[jObj].replace(/,/g, '')); 
                    if(Number.isNaN(this.cash)){this.cash=0; this.error=true; cashError=true;} break;
                default: if(jsonIn[jObj]!=="" && jsonIn[jObj]!==null && jsonIn[jObj]!==undefined) {this.items.push(new Item(jsonIn[jObj], this.mainChannel, this.utility));}
            }
        }
        if(nameFound===false){this.inde=_index; this.error=true; this.utility.sendMsg(msg.channel,"Character name not found for character " + _index);}
    ...
    async claim(msg){
        ...
        try{await member.setNickname(this.charName)}catch{e => this.utility.sendMsg(msg.channel,"Please change your nickname to " + this.charName)};
        ...
        });}catch{this.utility.sendMsg(msg.channel,"ERROR: Could not create new channel. Please ask the GMs to make one for you");this.channel=msg.author;}}   
        this.utility.sendMsg(msg.channel,"You have claimed character " + this.nameID);     
    }
    ...
    async release(msg){
        if(this.userID!==undefined){
            ...
            if(this.channel!==member){
                try{await this.channel.delete();}catch{this.utility.sendMsg(member,"Could not delete your channel; please ask the GMs to do this")}}
            this.userOldName=null;  this.userID=undefined;
            this.utility.sendMsg(msg.channel,"You have released character " + this.nameID);
        }
    }

    view(msg, msgInfo1, msgInfo2, gmFlag){
        if(this.items.length===0 && this.examineItems.length===0){
            this.utility.sendMsg(msg.channel,this.nameID + " has no items"); 
            if(msgInfo2===""){this.utility.sendMsg(msg.channel,this.cash===0 ? this.charName + " has no cash" : this.charName + " has " + this.cash.toLocaleString('en-GB') + " in cash");}
            return;
        }
        var findItem;   msgInfo1=msgInfo1.toLowerCase();
        if(msgInfo1!=="" && msgInfo1!=="all"){
            findItem = this.items.find(x=>x.nameID.toLowerCase()===msgInfo1.toLowerCase());
            if(findItem===undefined){findItem=this.examineItems.find(x=>x.nameID.toLowerCase()===msgInfo1.toLowerCase());}
            if(findItem===undefined){
                if(this.items.filter(x => x.infos.find(y=>y.heading.toLowerCase()===msgInfo1)).length===0
                    &&this.examineItems.filter(x => x.infos.find(y=>y.heading.toLowerCase()===msgInfo1)).length===0){
                        this.utility.sendMsg(msg.channel,"Cannot find this item name or information code among " + this.nameID + "'s items");
                        return;
                    }else{msgInfo2=msgInfo1;}
            }else{findItem.view(msg,msgInfo2, (msgInfo2.toLowerCase()==="all" && gmFlag)); return;}
        }
        this.utility.sendMsg(msg.channel,this.nameID + " has the following items:");
        for(var i=0;i<this.items.length;i++){
            this.items[i].view(msg, msgInfo2, (gmFlag && msgInfo1==="all"));
        }
        if(this.examineItems.length!==0){
            this.utility.sendMsg(msg.channel,this.nameID + " does not own but is currently examining the following items:");
            for(var i=0;i<this.examineItems.length;i++){
                this.examineItems[i].view(msg, msgInfo2, (gmFlag && msgInfo1.toLowerCase()==="all"));
            }
        }
        if(msgInfo2===""){this.utility.sendMsg(msg.channel,this.cash===0 ? this.charName + " has no cash" : this.charName + " has " + this.cash.toLocaleString('en-GB') + " in cash");}
    }
    ...
    give(msg, char2, msgInfo2){
        ...
        this.utility.sendMsg(msg.channel,"You have given item " + msgInfo2 + " to " + char2.charName);
    }
    async receive(msg, item){
        ...
        catch{this.utility.sendMsg(msg.channel,"Error: could not send message to " + this.charName + "'s channel - please let them know!");}}
    }
    show(msg, char2, msgInfo2){
        var itemIndex=this.items.findIndex(x=>x.nameID.toLowerCase()===msgInfo2.toLowerCase());
        if(itemIndex===-1){this.utility.sendMsg(msg.channel,"Error: Cannot find that item, have you typed the ID name correctly?"); return;}
        char2.beShown(msg, this.items[itemIndex]);
        this.utility.sendMsg(msg.channel,"You have shown item " + msgInfo2 + " to character " + char2.charName + ". They now have 3 minutes to examine it.");
    }
    ...
    pay(msg, payee, msgInfo2){
        ...
        if(msgInfo2<0){this.utility.sendMsg(msg.channel,"Error: you can't give a negative amount"); return;}
        if(msgInfo2>this.cash){this.utility.sendMsg(msg.channel,"Error: you can't give more money than you have"); return;}
        payee.getPaid(msg, msgInfo2);
        this.cash=this.cash-msgInfo2;
        this.utility.sendMsg(msg.channel,this.charName + " has given " + msgInfo2.toLocaleString('en-GB') + " to " + payee.charName);
    }
    async getPaid(msg, amount){
        this.cash=this.cash+amount;
        if(this.channel!==undefined){try{await this.channel.send("<@" + this.userID + "> " +"You have been given " + amount.toLocaleString('en-GB') + " in cash");}
        catch{this.utility.sendMsg(msg.channel,"Error: could not send message to " + this.charName + "'s channel - please let them know!");}}
    }
    ...
    renameNickname(msg, msgInfo2){
        ...
        this.utility.sendMsg(msg.channel,"Discord nickname changed from " + oldName + " to " + this.charName + " for character " + this.nameID);
    }
    async addItem(msg, msgInfo2){
        var newItem=new Item(msgInfo2, this.mainChannel, this.utility);
        this.items.push(newItem);
        await newItem.setup(msg);
        this.utility.sendMsg(msg.channel,"New item created. If there was a csv input file, it will fill in the info");
    }
